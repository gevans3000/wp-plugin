...
function sumai_activate() {
    $defaults = ['feed_urls'=>'','context_prompt'=>"Summarize the key points concisely.",'title_prompt'=>"Generate a compelling title.",'api_key'=>'','draft_mode'=>0,'schedule_time'=>'03:00','post_signature'=>''];
...
function sumai_check_external_trigger() {
    ...
    if ($stored && hash_equals($stored, $provided)) {
        ...
        if (false === get_transient($lock_key)) {
            ...
            // Call main function directly, it handles dependencies
            do_action(SUMAI_CRON_HOOK);
            // Consider exit() here if this URL is ONLY for the trigger
        } else {
            sumai_log_event('External trigger skipped, lock active.', true);
        }
    } else {
        sumai_log_event('Invalid external trigger token.', true);
    }
}

// --- Main Summary Generation ---
function sumai_generate_daily_summary( bool $force_fetch = false ) {
    // --- FORCE LOAD ADMIN FILES --- Critical fix for background tasks
    if ( ! function_exists( 'wp_insert_post' ) ) {
        $admin_inc = ABSPATH . 'wp-admin/includes/';
        if (file_exists($admin_inc.'admin.php')) require_once $admin_inc.'admin.php';
        if (file_exists($admin_inc.'post.php')) require_once $admin_inc.'post.php';
        if ( ! function_exists( 'wp_insert_post' ) ) {
            sumai_log_event( 'FATAL: Failed to load wp-admin/includes/post.php functions. Aborting.', true );
            return false;
        }
        sumai_log_event('Loaded admin includes for background task.');
    }
    // --- END FORCE LOAD ---
    ...
    try {
        ...
        sumai_log_event('Fetched '.mb_strlen($new_content).' chars of new content.');
        ...
        $unique_title = wp_unique_post_title($clean_title); // Function guaranteed by check above
        ...
        if (!$author || !$author->has_cap('publish_posts')) {
            ...
            $author = get_userdata($author_id);
            if (!$author || !$author->has_cap('publish_posts')) { sumai_log_event("Error: Author ID {$author_id} invalid or cannot publish.", true); return false; }
        }

        $post_data = [
            'post_title'   => $unique_title,
            'post_content' => $summary_result['content'], // Signature added by filter
            'post_status'  => ($options['draft_mode'] ?? 0) ? 'draft' : 'publish',
            'post_type'    => 'post',
            'post_author'  => $author_id,
            'meta_input'   => ['_sumai_generated' => true]
        ];
        $post_id = wp_insert_post($post_data, true); // Function guaranteed by check above
        ...
        if (!empty($guids_to_add)) {
            ...
            sumai_log_event("Processed GUIDs updated. Added: ".count($guids_to_add).", Pruned: {$pruned}. Total: ".count($guids));
        }
        return $post_id;
    } catch (\Throwable $e) {
        sumai_log_event("FATAL during generation: ".$e->getMessage()." L".$e->getLine()." F".$e->getFile(), true);
        return false;
    }
}
...
function sumai_fetch_new_articles_content( array $feed_urls, bool $force_fetch = false ): array {
    ...
    foreach ($feed_urls as $url) {
        ...
        foreach ($items as $item) {
            ...
            if (($char_count + $item_len) > SUMAI_MAX_INPUT_CHARS) { sumai_log_event("Skipping item '{$item_title}' - exceeds max input chars."); break; }
...
function sumai_summarize_text( string $text, string $ctx_prompt, string $title_prompt, string $api_key ): ?array {
    if (empty($text)) { sumai_log_event('Error: Empty text for summarization.', true); return null; }
    if (mb_strlen($text) > SUMAI_MAX_INPUT_CHARS) $text = mb_substr($text, 0, SUMAI_MAX_INPUT_CHARS); // Failsafe truncate

    $messages = [[ 'role' => 'system',
                   'content' => "You are an expert summarizer. Output valid JSON {\"title\": \"...\", \"summary\": \"...\"}. Context: ".($ctx_prompt ?: "Summarize key points concisely."). " Title Context: ".($title_prompt ?: "Generate a compelling title.") ],
    ...
    if (json_last_error() !== JSON_ERROR_NONE || !is_array($parsed) || empty($parsed['title']) || !isset($parsed['summary'])) {
        sumai_log_event('Error: Failed to parse valid JSON title/summary from API. Raw: '.$json_str, true); return null;
    }
...
function sumai_get_api_key(): string {
    ...
    if (defined('SUMAI_OPENAI_API_KEY') && !empty(SUMAI_OPENAI_API_KEY)) { $key = SUMAI_OPENAI_API_KEY; return $key; }
    $opts = get_option(SUMAI_SETTINGS_OPTION); $enc = $opts['api_key'] ?? '';
    if (empty($enc) || !function_exists('openssl_decrypt') || !defined('AUTH_KEY') || !AUTH_KEY) { $key = ''; return $key; }
    $decoded = base64_decode($enc, true); $cipher = 'aes-256-cbc'; $ivlen = openssl_cipher_iv_length($cipher);
    if ($decoded === false || $ivlen === false || strlen($decoded) <= $ivlen) { sumai_log_event('Error: Invalid stored API key format/decode.', true); $key = ''; return $key; }
    $iv = substr($decoded, 0, $ivlen); $cipher_raw = substr($decoded, $ivlen);
    $decrypted = openssl_decrypt($cipher_raw, $cipher, AUTH_KEY, OPENSSL_RAW_DATA, $iv);
    if ($decrypted === false) { sumai_log_event('Error: Failed to decrypt API key.', true); $key = ''; return $key; }
    $key = $decrypted; return $key;
}
...
function sumai_sanitize_settings($input): array {
    ...
    // Feeds
    $valid_urls = []; $invalid = false;
    if (isset($input['feed_urls'])) {
        $urls = array_map('trim', preg_split('/\r\n|\r|\n/', sanitize_textarea_field($input['feed_urls'])));
        foreach ($urls as $url) { if (empty($url)) continue; if (filter_var($url, FILTER_VALIDATE_URL) && preg_match('/^https?:\/\//', $url)) $valid_urls[] = $url; else $invalid = true; }
        if ($invalid) add_settings_error(SUMAI_SETTINGS_OPTION, 'invalid_feed', 'Invalid feed URLs removed.', 'warning');
        if (count($valid_urls) > SUMAI_MAX_FEED_URLS) { $valid_urls = array_slice($valid_urls, 0, SUMAI_MAX_FEED_URLS); add_settings_error(SUMAI_SETTINGS_OPTION, 'feed_limit', 'Feed URL limit reached.', 'warning'); }
    }
    ...
    // API Key
    if (defined('SUMAI_OPENAI_API_KEY') && !empty(SUMAI_OPENAI_API_KEY)) { $sanitized['api_key'] = $current_enc; /* Keep DB value if constant set, but constant overrides */ }
    elseif (isset($input['api_key'])) {
        ...
        elseif (empty($new_key_in)) { $sanitized['api_key'] = ''; if (!empty($current_enc)) add_settings_error(SUMAI_SETTINGS_OPTION, 'api_clear', 'API key cleared.', 'updated'); }
        else { // New key provided
            if (!function_exists('openssl_encrypt') || !defined('AUTH_KEY') || !AUTH_KEY) { add_settings_error(SUMAI_SETTINGS_OPTION, 'api_enc_err', 'Cannot encrypt API key (OpenSSL/AUTH_KEY missing). Key NOT saved.', 'error'); $sanitized['api_key'] = $current_enc; }
            else {
                $cipher = 'aes-256-cbc'; $ivlen = openssl_cipher_iv_length($cipher);
                if ($ivlen === false) { add_settings_error(SUMAI_SETTINGS_OPTION, 'api_enc_err', 'Cannot get IV length. Key NOT saved.', 'error'); $sanitized['api_key'] = $current_enc; }
                else {
                    $iv = openssl_random_pseudo_bytes($ivlen); $enc = openssl_encrypt($new_key_in, $cipher, AUTH_KEY, OPENSSL_RAW_DATA, $iv);
                    if ($enc !== false && $iv !== false) { $new_enc = base64_encode($iv.$enc); if ($new_enc !== $current_enc) { add_settings_error(SUMAI_SETTINGS_OPTION, 'api_saved', 'New API key saved.', 'updated'); sumai_log_event('API key saved via settings.'); } $sanitized['api_key'] = $new_enc; }
                    else { add_settings_error(SUMAI_SETTINGS_OPTION, 'api_enc_err', 'Failed to encrypt API key. Key NOT saved.', 'error'); $sanitized['api_key'] = $current_enc; }
                }
            }
        }
    } else $sanitized['api_key'] = $current_enc; // Not submitted, keep current
...
function sumai_render_settings_page() {
    ...
    if ('POST' === $_SERVER['REQUEST_METHOD'] && isset($_POST['sumai_generate_now']) && check_admin_referer('sumai_generate_now_action')) {
        sumai_log_event('Manual generation triggered.');
        if (!function_exists('wp_insert_post')) require_once ABSPATH.'wp-admin/includes/post.php'; // Ensure needed file
        ...
        add_settings_error('sumai_settings', 'manual_gen', $msg_text, $msg_type);
        set_transient('settings_errors', get_settings_errors(), 30); // Persist notices
    ...
    $opts = wp_parse_args($opts, ['feed_urls'=>'','context_prompt'=>'','title_prompt'=>'','api_key'=>'','draft_mode'=>0,'schedule_time'=>'03:00','post_signature'=>'']);
    $const_key = defined('SUMAI_OPENAI_API_KEY') && !empty(SUMAI_OPENAI_API_KEY);
    $db_key_set = !empty($opts['api_key']);
    $api_disp = $const_key ? '*** Defined in wp-config.php ***' : ($db_key_set ? '********************' : '');
    ?>
    ...
    <tr><th><label for="sf_urls">Feed URLs</label></th><td><textarea id="sf_urls" name="<?= esc_attr(SUMAI_SETTINGS_OPTION) ?>[feed_urls]" rows="3" class="large-text" placeholder="One feed URL per line (http/https)"><?= esc_textarea($opts['feed_urls']) ?></textarea><p class="description">Max <?= SUMAI_MAX_FEED_URLS ?> feeds.</p></td></tr>
    <tr><th><label for="sf_key">OpenAI API Key</label></th><td>
        <?php if ($const_key): ?><input type="text" value="<?= esc_attr($api_disp) ?>" class="regular-text" readonly disabled /><p class="description">Key defined in wp-config.php.</p>
        <?php else: ?><input type="password" id="sf_key" name="<?= esc_attr(SUMAI_SETTINGS_OPTION) ?>[api_key]" value="<?= esc_attr($api_disp) ?>" class="regular-text" placeholder="<?= $db_key_set ? 'Enter new key to update' : 'Enter API Key' ?>" autocomplete="new-password" />
        <button type="button" id="test-api-btn" class="button">Test Key</button><span id="api-test-res" style="margin-left:10px;"></span>
        <p class="description">Securely stored. <?= $db_key_set ? 'Leave stars to keep, enter new to replace, clear to remove.' : '<a href="https://platform.openai.com/api-keys" target="_blank">Get key</a>.' ?>
        <?php if (!function_exists('openssl_encrypt')||!defined('AUTH_KEY')||!AUTH_KEY) echo '<br><strong style="color:red;">Warning: Cannot encrypt key in DB. Use wp-config.php constant.</strong>'; ?></p>
        <?php endif; ?>
    </td></tr>
    <tr><th><label for="sf_ctx">Summary Prompt</label></th><td><textarea id="sf_ctx" name="<?= esc_attr(SUMAI_SETTINGS_OPTION) ?>[context_prompt]" rows="3" class="large-text" placeholder="e.g., Summarize concisely."><?= esc_textarea($opts['context_prompt']) ?></textarea><p class="description">Optional AI instructions.</p></td></tr>
    <tr><th><label for="sf_ttl">Title Prompt</label></th><td><textarea id="sf_ttl" name="<?= esc_attr(SUMAI_SETTINGS_OPTION) ?>[title_prompt]" rows="2" class="large-text" placeholder="e.g., Create compelling title."><?= esc_textarea($opts['title_prompt']) ?></textarea><p class="description">Optional AI instructions.</p></td></tr>
    <tr><th>Publish Status</th><td><fieldset><label><input type="radio" name="<?= esc_attr(SUMAI_SETTINGS_OPTION) ?>[draft_mode]" value="0" <?php checked(0,$opts['draft_mode']) ?>> Publish</label><br><label><input type="radio" name="<?= esc_attr(SUMAI_SETTINGS_OPTION) ?>[draft_mode]" value="1" <?php checked(1,$opts['draft_mode']) ?>> Draft</label></fieldset></td></tr>
    <tr><th><label for="sf_time">Schedule Time</label></th><td><input type="time" id="sf_time" name="<?= esc_attr(SUMAI_SETTINGS_OPTION) ?>[schedule_time]" value="<?= esc_attr($opts['schedule_time']) ?>" class="regular-text" required pattern="([01]?\d|2[0-3]):[0-5]\d" /><p class="description">Daily generation (HH:MM, 24h). Timezone: <strong><?= esc_html(wp_timezone_string()) ?></strong>.<?php $next = wp_next_scheduled(SUMAI_CRON_HOOK); echo '<br>Next run: '.($next ? wp_date(get_option('date_format').' '.get_option('time_format'), $next) : 'Not scheduled.'); ?></p></td></tr>
    <tr><th><label for="sf_sig">Post Signature</label></th><td><textarea id="sf_sig" name="<?= esc_attr(SUMAI_SETTINGS_OPTION) ?>[post_signature]" rows="3" class="large-text" placeholder="Optional HTML/Text"><?= esc_textarea($opts['post_signature']) ?></textarea><p class="description">Appended to posts. Basic HTML allowed.</p></td></tr>
    </table><?php submit_button('Save Settings'); ?></form></div>
    <div id="tab-advanced" class="tab-content" style="display:none;"><h2>Advanced</h2><div class="card"><h3>Generate Now</h3><p>Manually trigger summary generation (force fetch).</p><form method="post"><input type="submit" name="sumai_generate_now" class="button button-primary" value="Generate Now"><?php wp_nonce_field('sumai_generate_now_action'); ?></form></div><div class="card"><h3>Test Feeds</h3><p>Check feed access and see new items.</p><button type="button" id="test-feed-btn" class="button">Test Feeds</button><div id="feed-test-res" class="res-box"></div></div><div class="card"><h3>External Cron</h3><p>Use this URL for external cron services:</p><?php $token = get_option(SUMAI_CRON_TOKEN_OPTION); if ($token) { $url = add_query_arg(['sumai_trigger'=>'1','token'=>$token], site_url('/')); echo '<input type="text" value="'.esc_url($url).'" class="large-text" readonly onfocus="this.select();"><p class="description">e.g., <code>wget -qO- \''.esc_url($url).'\' > /dev/null</code></p>'; } else echo '<p>Token not generated. Save settings.</p>'; ?></div></div>
    <div id="tab-debug" class="tab-content" style="display:none;"><h2>Debug Info</h2><?php sumai_render_debug_info(); ?></div></div></div>
    <style>.card{padding:15px 20px;border:1px solid #ccd0d4;background:#fff;margin:20px 0;box-shadow:0 1px 1px rgba(0,0,0,.04);}.res-box{margin-top:10px;padding:15px;background:#f6f7f7;border:1px solid #ccd0d4;max-height:400px;overflow-y:auto;display:none;white-space:pre-wrap;font-family:monospace;font-size:12px;}.nav-tab-wrapper{margin-bottom:20px;}</style>
    ...
    $('#test-api-btn').on('click',function(){var $btn=$(this),$res=$('#api-test-res'),$input=$('#sf_key'),key='';if($input.length)key=($input.val()==='********************')?'':$input.val();$btn.prop('disabled',true).text('Testing...');$res.html('<span class="spinner is-active" style="vertical-align:middle;margin-right:5px;"></span>Testing...').css('color','');$.post(ajaxurl,{action:'sumai_test_api_key',_ajax_nonce:'<?=wp_create_nonce('sumai_test_api_key_nonce')?>',api_key_to_test:key},function(r){if(r.success)$res.html('✅ '+r.data.message).css('color','green');else $res.html('❌ '+r.data.message).css('color','#d63638');},'json').fail(function(){$res.html('❌ AJAX Error').css('color','#d63638');}).always(function(){$btn.prop('disabled',false).text('Test Key');});});
    $('#test-feed-btn').on('click',function(){var $btn=$(this),$res=$('#feed-test-res');$btn.prop('disabled',true).text('Testing...');$res.html('<span class="spinner is-active" style="vertical-align:middle;margin-right:5px;"></span>Testing...').css('color','').show();$.post(ajaxurl,{action:'sumai_test_feeds',_ajax_nonce:'<?=wp_create_nonce('sumai_test_feeds_nonce')?>'},function(r){if(r.success)$res.html(r.data.message).css('color','');else $res.html('❌ Error: '+r.data.message).css('color','#d63638');},'json').fail(function(){$res.html('❌ AJAX Error').css('color','#d63638');}).always(function(){$btn.prop('disabled',false).text('Test Feeds');});});});</script>
    <?php
}

// --- AJAX Handlers ---
add_action('wp_ajax_sumai_test_api_key', function(){ check_ajax_referer('sumai_test_api_key_nonce'); if(!current_user_can('manage_options'))wp_send_json_error(['message'=>'Denied.'],403); $in=isset($_POST['api_key_to_test'])?trim(sanitize_text_field($_POST['api_key_to_test'])):''; $key=empty($in)?sumai_get_api_key():$in; $ctx=empty($in)?'Current key':'Provided key'; if(empty($key))wp_send_json_error(['message'=>'API key not configured.']); if(sumai_validate_api_key($key))wp_send_json_success(['message'=>$ctx.' validation OK.']); else wp_send_json_error(['message'=>$ctx.' validation FAILED. Check logs.']); });
add_action('wp_ajax_sumai_test_feeds', function(){ check_ajax_referer('sumai_test_feeds_nonce'); if(!current_user_can('manage_options'))wp_send_json_error(['message'=>'Denied.'],403); $opts=get_option(SUMAI_SETTINGS_OPTION,[]); $urls=empty($opts['feed_urls'])?[]:array_slice(array_filter(array_map('trim',preg_split('/\r\n|\r|\n/',$opts['feed_urls']))),0,SUMAI_MAX_FEED_URLS); if(empty($urls))wp_send_json_error(['message'=>'No feeds configured.']); if(!function_exists('fetch_feed')){include_once ABSPATH.WPINC.'/feed.php';if(!function_exists('fetch_feed'))wp_send_json_error(['message'=>'WP feed functions unavailable.']);} $out=sumai_test_feeds($urls); wp_send_json_success(['message'=>'<pre>'.esc_html($out).'</pre>']); });
...
function sumai_test_feeds(array $feed_urls): string {
    ...
    foreach ($feed_urls as $i => $url) {
        ...
        if (!$feed_new) $out .= "  ℹ️ No new items in latest checked.\n"; $out .= "\n"; unset($feed, $items);
...
function sumai_ensure_log_dir(): ?string {
    ...
    if(!is_writable($dir)){error_log("Sumai Log Err: Dir not writable {$dir}"); return null;}
    if(!file_exists($file)){if(false===@file_put_contents($file,'')){error_log("Sumai Log Err: Failed create file {$file}"); return null;} @chmod($file,0644);}
    if(!is_writable($file)){error_log("Sumai Log Err: File not writable {$file}"); return null;} $path=$file; return $path;
}
...
function sumai_prune_logs() { $days=SUMAI_LOG_TTL/DAY_IN_SECONDS; sumai_log_event("Running log pruning (keep {$days}d)..."); $file=sumai_ensure_log_dir(); if(!$file||!is_readable($file)||!is_writable($file)){sumai_log_event('Log pruning skipped: file inaccessible.',true);return;} $lines=@file($file,FILE_IGNORE_NEW_LINES|FILE_SKIP_EMPTY_LINES); if(empty($lines)){sumai_log_event('Log pruning: file empty/unreadable.');return;} $cutoff=time()-SUMAI_LOG_TTL; $keep=[]; $pruned=0; $total=count($lines);
    foreach($lines as $line){ $ts=false; if(preg_match('/^\[(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2} [A-Z\/+-\w\:]+)\]/',$line,$m)){try{$dt=new DateTime($m[1]);$ts=$dt->getTimestamp();}catch(Exception $e){$ts=strtotime($m[1]);}} if($ts!==false&&$ts>=$cutoff)$keep[]=$line; else $pruned++;}
    if($pruned>0){$new_content=empty($keep)?'':implode(PHP_EOL,$keep).PHP_EOL; if(false!==@file_put_contents($file,$new_content,LOCK_EX))sumai_log_event("Log pruned. Processed {$total}, removed {$pruned}."); else sumai_log_event("Log pruning failed: Could not write file.",true);} else sumai_log_event("Log pruning: Processed {$total}, no old entries found.");
}
...
function sumai_get_debug_info(): array {
    ...
    $file=sumai_ensure_log_dir(); $dbg['log']=['path'=>$file?:'ERROR','writable'=>$file&&is_writable($file),'readable'=>$file&&is_readable($file)]; if($dbg['log']['readable']){$cont=@file_get_contents($file,false,null,-10240);$dbg['log']['recent']=($cont!==false)?array_slice(explode("\n",trim($cont)),-30):['Error reading log.'];}else $dbg['log']['recent']=['Log not readable.'];
...
function sumai_render_debug_info() { $dbg=sumai_get_debug_info(); echo '<div><style>td{vertical-align:top;}pre{white-space:pre-wrap;word-break:break-all;background:#f6f7f7;padding:5px;border:1px solid #ccc;margin:0;font-size:12px;}</style>';
    echo '<h3>Settings</h3><table class="wp-list-table widefat fixed striped"><tbody>'; foreach($dbg['settings'] as $k=>$v) echo '<tr><td width="30%"><strong>'.esc_html(ucwords(str_replace('_',' ',$k))).'</strong></td><td><pre>'.esc_html(is_array($v)?print_r($v,true):$v).'</pre></td></tr>'; echo '</tbody></table>';
    echo '<h3>Scheduled Tasks</h3>'; if(is_array($dbg['cron'])&&!empty($dbg['cron'])){echo '<table class="wp-list-table widefat fixed striped"><thead><tr><th>Hook</th><th>Next Run (Site Time)</th><th>Schedule</th></tr></thead><tbody>'; foreach($dbg['cron'] as $h=>$d) echo '<tr><td><code>'.esc_html($h).'</code></td><td>'.esc_html($d['next']).'</td><td>'.esc_html($d['schedule']).'</td></tr>'; echo '</tbody></table>';} else echo '<p>'.esc_html($dbg['cron']).'</p>';
    echo '<h3>System</h3><table class="wp-list-table widefat fixed striped"><tbody>'; foreach($dbg['sys'] as $k=>$v) echo '<tr><td width="30%"><strong>'.esc_html(strtoupper($k)).'</strong></td><td>'.esc_html($v).'</td></tr>'; echo '</tbody></table>';
    echo '<h3>Logging</h3><table class="wp-list-table widefat fixed striped"><tbody><tr><td width="30%"><strong>Path</strong></td><td><code>'.esc_html($dbg['log']['path']).'</code></td></tr><tr><td><strong>Status</strong></td><td>'.($dbg['log']['readable']?'Readable':'Not Readable').', '.($dbg['log']['writable']?'Writable':'Not Writable').'</td></tr></tbody></table><h4>Recent Logs (tail)</h4><pre>'.esc_html(implode("\n",$dbg['log']['recent'])).'</pre>';
    echo '</div>';
}
